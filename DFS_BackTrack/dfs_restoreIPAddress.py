...
class Solution(object):
    ...
    def dfs(self, source_string_leftover, count, current_ip_part):
        # 0. result ip address should have 4 ip parts.
        if count == 4:
        ...
        for i in range(1, 4):
            ...
            if i <= len(source_string_leftover):
                ...
                if int(source_string_leftover[:i]) <= 255:
                    # this will make the result like ".111.244.55.123",
                    # so you have to remove the first "." when add the ip to result set.
                    self.dfs(source_string_leftover[i:], count+1, current_ip_part + "." + source_string_leftover[i:])
                # 2. each ip part cannot be starting with 0, except only 0. For example, no "01.123.123.123"
